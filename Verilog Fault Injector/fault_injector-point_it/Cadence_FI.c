...
//			printf("Port OBJ : %s --> %d \n", vpi_get_str(vpiFullName, find_handle), vpi_get(vpiSize, find_handle));
			...
			// ACE 분석에서는 결함주입 위치를  직접 설정한다.
			//partial_boundary++;
			//fault_injection_check(find_handle);
			

			if(strcmp(partial_module_name, vpi_get_str(vpiFullName, find_handle)) == 0)
			{
						
				fault_configuration_list[0].fault_handle =find_handle;
...
int fault_enable(p_cb_data cb_data_p)
{
	...
	fault_configuration_list[x].fault_location_incorrect_value[fault_configuration_list[x].fault_range % 38] = _fault;
...
/*
	...
	// 현재 값 backup
		...
		if(fault_configuration_list[x].fault_location_vector_size <= fault_configuration_list[x].fault_range)// data weith < fault range
		{
			...
			// all fault bit	vpi_printf(" ==>  test 1\n");
			int k;
...
vpi_printf(" ----------->  test	: %s \n", vpi_get_str(vpiFullName, fault_configuration_list[x].fault_handle));
vpi_printf(" ----------->  test : %s \n", f_value_p->value.str);
...
{
      vpi_printf("\n\n ICARUS Fault Injection, from VPI for IC_fault checksum\n");
...
vpi_printf(" ----------->  TopModule	: %s \n", vpi_get_str(vpiFullName, top_module));
	...
	if(fault_location_type == 2)
	{
		vpi_printf("*** partial_fault_boundary => %d \n", partial_boundary);
...
fprintf(trace, " ==> Fault location 	: %s \n", vpi_get_str(vpiFullName, fault_configuration_list[i].fault_handle));
	...
	if(fault_injection_enable ==1)
	{
		for(i = 0; i<fault_list_total_num ; i++) 
		{
			...
			cb_fault_enable[i].reason		= cbAfterDelay;
		...
		//		time_fault_enable[i].low			= fault_configuration_list[i].fault_injection_time+1;
				time_fault_release[i].type 		= vpiSimTime;
			
				cb_fault_release[i].reason		= cbAfterDelay;
...
*/
	...
	top_load_module= vpi_handle_by_name("or1200_tb", NULL);
